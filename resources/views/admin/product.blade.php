            ...
            <div class="col-md-8 mx-auto">
                <form action="{{route('product.update',$product->slug)}}" method="POST" enctype="multipart/form-data">
                            ...
                            @foreach($cats as $key=>$cat)
                                ...
                                @if(count($cat['subcat'])>0)
                                    @foreach($cat['subcat'] as $subkey=>$subcat)
                                        @if($subcat['id'] == $productcat->id)
                                        
                                            <option value="{{$subcat['id']}}" data-parent_id="{{$cat['id']}}" data-cat="{{$subcat['id']}}" selected>---{{$subkey}}</option>
                                        @else
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="title" id="title" class="form-control @error('title') is-invalid @enderror" value="{{$product->name}}">
                        @error('title')
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="content" id="editor" class="form-control @error('content') is-invalid @enderror" cols="30" rows="10">{{$product->content}}</textarea>
                        @error('content')
                            <strong class="invalid-feedback">
                                {{$message}}
                            </strong>
